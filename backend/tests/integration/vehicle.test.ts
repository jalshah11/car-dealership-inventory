...
// wiring works (a request with a USER token really does get 403 on an
// admin-only route), not just that a mock was configured to allow it.
...
describe('Vehicle routes', () => {
  ...
  describe('GET /api/vehicles', () => {
    it('returns 200 and a list, with no auth required', async () => {
  ...
  describe('GET /api/vehicles/search', () => {
    ...
    it('coerces and applies minPrice/maxPrice from query strings', async () => {
      ...
      await request(app).get('/api/vehicles/search?minPrice=10000&maxPrice=50000');
    ...
    it('returns 400 for a non-numeric minPrice', async () => {
      const response = await request(app).get('/api/vehicles/search?minPrice=abc');
      expect(response.status).toBe(400);
    });
  });

  describe('GET /api/vehicles/:id', () => {
    it('returns 200 and the vehicle when found', async () => {
    ...
    it('returns 404 when not found', async () => {
      ...
      const response = await request(app).get('/api/vehicles/missing-id');
  ...
  describe('POST /api/vehicles/:id/restock', () => {
    ...
    it('returns 403 for a non-admin user, even though purchase allows them', async () => {
      const response = await request(app)
        .post('/api/vehicles/vehicle-1/restock')
        .set('Authorization', userToken)
        .send({ amount: 10 });
